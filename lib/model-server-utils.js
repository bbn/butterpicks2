(function() {
  ...
  League.fetchOrCreateForStatsKey = function(statsKey, options) {
    return League.fetchForStatsKey(statsKey, {
      error: options.error,
      success: function(league, response) {
        if (league) return options.success(league);
        league = new League({
          statsKey: statsKey
        });
        return league.save(league.toJSON(), {
          error: options.error,
  ...
  League.fetchById = function(options) {
    var league;
    league = new League({
      _id: options.id
    });
    return league.fetch(options);
  };
  ...
  Game.fetchByStatsKey = function(options) {
    var g;
    g = new Game({
      _id: Game.couchIdForStatsKey(options.statsKey)
    });
    return g.fetch({
      error: options.error,
        ...
        return Game.fetchByStatsKey({
          ...
          success: function(game, response) {
            return game.fetchBasePeriodId({
              error: options.error,
              success: function(oldBasePeriodId) {
                return game.save(attributes, {
                  error: options.error,
                  success: function(game) {
                    return PeriodUpdateJob.create({
                      ...
                      success: function() {
                        return game.fetchBasePeriodId({
                          ...
                          success: function(newBasePeriodId) {
                            if (newBasePeriodId === oldBasePeriodId) {
                              return options.success(game);
                            }
                            return PeriodUpdateJob.create({
