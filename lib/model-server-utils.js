(function() {
  ...
  UserPeriod.prototype.fetchPicks = function(options) {
    var errorReturned, game, picks, _i, _len, _ref, _results,
      _this = this;
    if (!this.games) return options.error("games not loaded");
    if (!this.games.length) return options.success([]);
    picks = [];
    errorReturned = false;
    _ref = this.games;
    _results = [];
      ...
      _results.push((function(game) {
        return Pick.fetchForUserAndGame({
          ...
          success: function(pick) {
            ...
            picks.push(pick);
            if (picks.length === _this.games.length) return options.success(picks);
          }
        });
      })(game));
    }
