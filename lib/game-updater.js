(function() {
  var Game, couch, getMostRecentlyUpdatedGameDate, models, pollInterval, request, updateGames, util;
  ...
  exports.poll(function(interval) {
    if (interval) pollInterval = interval;
    if (pollInterval) {
      return setTimeout(updateGames({
        poll: true
      }), pollInterval);
    }
  });
    ...
    return getMostRecentlyUpdatedGameDate({
      ...
      success: function(lastUpdatedGameDate, response) {
        ...
        return request.get(requestParams, function(error, response, body) {
          ...
          if (options && options.poll) return exports.poll();
        });
      }
    });
  };
