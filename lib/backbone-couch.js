(function() {
  ...
  transformAttributesForSaving = function(attributes) {
    ...
    for (key in attributes) {
      ...
      val = attributes[key];
      if (key.match(/Date$/) && val) attributes[key] = val.toJSON();
    }
    if (attributes.id) attributes._id = attributes.id;
    return attributes;
  };
  ...
  tranformAttributesFromFetching = function(attributes) {
    ...
    for (key in attributes) {
      if (!__hasProp.call(attributes, key)) continue;
      val = attributes[key];
      if (key.match(/Date$/) && val) attributes[key] = new Date(val);
    }
    if (attributes._id) attributes.id = attributes._id;
    return attributes;
  };
